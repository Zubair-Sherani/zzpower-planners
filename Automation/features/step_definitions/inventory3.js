...
Given(/^I update the inventory level for the product with a invalid input$/, async function () {
    ...
    return await browser.click('td[name="inventory_quantity"]') // click on the initial field
        ...
        .setValue('input.o_input', 'abc');

});

When(/^i click clear$/, async function () {

    return await browser.click('button[name="action_set_inventory_quantity_to_zero"]'); // click on the "Apply" button;
});

Then(/^there should be an zero in field$/, async function () {
    return await browser.waitForElementVisible('div[name="inventory_quantity"]', 5000)
        .getValue('div[name="inventory_quantity"] input', function (result) {
            var value = isNaN(result.value) ? 0 : parseInt(result.value, 10);
            console.log('Inventory quantity:', value);
            this.assert.equal(value, 0);
        })
        .pause(10000)
        ;
});
