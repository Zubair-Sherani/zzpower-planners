...
Given(/^I update the inventory level for the product with a nagative input$/, async function () {
...
Then(/^then the quantity will not be updated$/, async function () {
    return await browser.waitForElementVisible('td[name="quantity"] div[name="quantity"] > span', 5000)
        .getText('td[name="quantity"] div[name="quantity"] > span', function (result) {
            ...
            this.assert.equal(result, '-99.00', 'Inventory level was not updated correctly.');
        });
});
