...
Then(/^there should be an error popup$/, async function () {
    return await browser.waitForElementVisible('[role="alert"]', 5000, false, function (result) {
        browser.getText('div[role="alert"] ul li:first-child', function (result) {
            this.assert.equal('true', 'true', 'popup not shown');
        });
    })
});
