...
Given(/^I update the inventory level for the product with a float input$/, async function () {
...
Then(/^the quantity will be integer update$/, async function () {
    return await browser.waitForElementVisible('td[name="quantity"]', 5000)
        .waitForElementVisible('.o_readonly_modifier', 5000)
        .waitForElementVisible('.span', 5000)
        .getText('span', function (result) {
            var numericRegex = /^[\d,.]+$/; // Only allow numeric characters, decimal points, and commas
            var value = numericRegex.test(result.value) ? parseFloat(result.value.replace(',', '.')) : NaN;
            this.assert.equal(value, 21.5);
        });
});
